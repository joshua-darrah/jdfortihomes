...
create table if not exists public.bookings (
  ...
  agent_fee numeric check (agent_fee is null or agent_fee >= 0),
  platform_commission_rate numeric not null default 15 check (platform_commission_rate >= 0 and platform_commission_rate <= 100),
...
alter table public.listings add column if not exists agent_fee numeric check (agent_fee is null or agent_fee >= 0);
alter table public.bookings add column if not exists agent_fee numeric check (agent_fee is null or agent_fee >= 0);
alter table public.bookings add column if not exists platform_commission_rate numeric not null default 15 check (platform_commission_rate >= 0 and platform_commission_rate <= 100);
alter table public.bookings add column if not exists platform_commission_amount numeric not null default 0 check (platform_commission_amount >= 0);
alter table public.bookings add column if not exists agent_payout_amount numeric not null default 0 check (agent_payout_amount >= 0);
...
-- Platform fee configuration. Admins can change these values without editing environment variables.
...
create policy "Authenticated users can read platform commission rate"
...
alter table public.agent_payouts add column if not exists booking_id uuid references public.bookings(id) on delete set null;
...
-- Agent-fee payout creation and direct-assignment synchronization.
...
begin
  ...
  insert into public.agent_payouts (
    booking_id, agent_id, listing_id, amount, currency, status, notes
  )
  values (
    new.id,
    assigned_agent,
...
create trigger create_agent_payout_for_booking
after insert on public.bookings
for each row execute procedure public.create_agent_payout_for_booking();
...
begin
  ...
  if booking_record.id is null or booking_record.agent_fee is null then
...
create trigger sync_payout_agent_from_booking_assignment
after insert or update or delete on public.booking_agents
for each row execute procedure public.sync_payout_agent_from_booking_assignment();

-- Targeted performance indexes. See supabase/performance-indexes.sql.
create index if not exists bookings_created_at_idx on public.bookings(created_at desc);
create index if not exists bookings_status_created_at_idx on public.bookings(status, created_at desc);
create index if not exists bookings_preferred_date_idx on public.bookings(preferred_date, preferred_time);
...
create index if not exists listings_property_type_status_idx on public.listings(property_type, status);
create index if not exists ads_status_ends_at_idx on public.ads(status, ends_at);
create index if not exists ads_listing_id_idx on public.ads(listing_id);
